...
<h1 class="text-[#212529] text-4xl sm:text-5xl font-bold tracking-tight">Modification de la  Citation <span class="text-sm">{{ $quote->content }}</span></h1>
<p class="text-gray-500 text-lg font-normal">Modifiez les champs ci-dessous pour mettre à jour la citation.</p>
</div>
</div>
<!-- Form Fields -->
<div class="flex flex-col gap-8 px-4">
    <form action="{{url('/edit/'.  $quote->id  . '/update')}}" method="post">
        @csrf
        @method('patch')
...
<textarea name="content" class="border border-gray-200 form-input flex w-full min-w-0 flex-1 resize-y overflow-hidden rounded-xl bg-transparent text-[#212529] focus:outline-0 focus:ring-0 border-none min-h-36 placeholder:text-gray-400 p-[15px] text-base font-normal leading-normal" placeholder="Saisissez ici la citation..."> {{ $quote->content }} </textarea>
</div>
</label>
<!-- Input for Author -->
<label class="flex flex-col w-full">
...
<div class="rounded-xl form-input-neon">
<input name="author" class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl bg-transparent text-[#212529] focus:outline-0 focus:ring-0 border-none h-14 placeholder:text-gray-400 p-[15px] text-base font-normal leading-normal" placeholder="Nom de l'auteur" value="{{ $quote->author }}"/>
</div>
</label>
</div>
...
<input type="checkbox" name="is_published" 
    @if($quote->is_published > 0) checked @endif>
