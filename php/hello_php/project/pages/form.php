...
}elseif(!filter_var($formData['date_naissance'] ?? '' , FILTER_SANITIZE_NUMBER_INT)){
    $errors[] = "La date de naissance n'est pas valide";
}

if(empty($formData['telephone'])){
    $errors[] = "Le telephone est obligatoire";
}elseif(!filter_var($formData['telephone'] ?? '' , FILTER_SANITIZE_NUMBER_INT)){
    $errors[] = "Le telephone n'est pas valide";
}

if(empty($formData['site_web'])){
    $errors[] = "Le site web est obligatoire";
}elseif(!filter_var($formData['site_web'] ?? '' , FILTER_SANITIZE_URL)){
    $errors[] = "Le site web n'est pas valide";
}

if(empty($formData['langues'])){
    $errors[] = "Le langues est obligatoire";
}

if(empty($formData['interets'])){
    $errors[] = "Le interets est obligatoire";
}
...
if(empty($formData['newsletter'])){
    $errors[] = "Le newsletter est obligatoire";
}elseif(!filter_var($formData['newsletter'] ?? '' , FILTER_SANITIZE_NUMBER_INT)){
    $errors[] = "Le newsletter n'est pas valide";
}
 
if(empty($formData['conditions'])){
    $errors[] = "Le conditions est obligatoire";
}elseif(!filter_var($formData['conditions'] ?? '' , FILTER_SANITIZE_NUMBER_INT)){
    $errors[] = "Le conditions n'est pas valide";
}
 
if(empty($formData['age'])){
    $errors[] = "Le age est obligatoire";
}elseif(!filter_var($formData['age'] ?? '' , FILTER_SANITIZE_NUMBER_INT)){
    $errors[] = "Le age n'est pas valide";
}
 
if(empty($formData['civilite'])){
    $errors[] = "Le civilite est obligatoire";
}
  ...
  return $data;
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" id="telephone" name="telephone" placeholder="0612345678"
                           pattern="[0-9]{10}" value="<?php echo htmlspecialchars($formData['telephone'] ?? ''); ?>" required>
                </div>
   ...
   if($_SERVER['REQUEST_METHOD'] === 'POST'):
    ...
    <div class="result-section">
        <h3>Récaptulatif des données soumises</h3>
        <p class="result-item"><span class="result-label"> Nom complet</span>
        <?= 
        $formData['civilite'] . ' ' . $formData['nom'] 
        ?>
        </p>
        <p class="result-item"><span class="result-label"> Email </span>
        <?= 
        $formData['email']
        ?>
        </p>
        <p class="result-item"><span class="result-label"> Téléphone </span>
        <?= 
        $formData['telephone']
        ?>
        </p>
        <p class="result-item"><span class="result-label"> Site web </span>
        <?= 
        $formData['site_web']
        ?>
        </p>
        <p class="result-item"><span class="result-label"> Langues parlées</span>
        <?= 
        implode(', ', $formData['langues'])
        ?>
        </p>
        <p class="result-item"><span class="result-label"> Centres d'intérêt </span>
        <?= 
        implode(', ', $formData['interets'])
        ?>
        </p>
        <p class="result-item"><span class="result-label"> Genre </span>
        <?= 
        $formData['genre']
        ?>
        </p>
        <p class="result-item"><span class="result-label"> Date de naissance </span>
        <?= 
        $formData["date_naissance"] . "(".$formData['age'].")"
        ?>
        </p>
        <p class="result-item"><span class="result-label">Newsletter</span>
        <?= 
        $formData['newsletter'] ? 'Oui' : 'Non'
        ?>
        </p>
        <p class="result-item"><span class="result-label">Conditions</span>
            
        <?= 
        $formData['conditions'] ? 'Oui' : 'Non'
        ?>
        </p>
    </div>
    <?php
